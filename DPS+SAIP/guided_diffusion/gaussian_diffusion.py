...
class GaussianDiffusion:
    def __init__(self,
        ...
        self.var_processor = get_var_processor(model_var_type,
                                               betas=betas)
        self.betas = torch.tensor(self.betas, dtype=torch.float32, device='cuda')  # 或 device='cuda' if你固定用了GPU
    ...
    def p_sample_loop(self,
        ...
        for idx in pbar:
            ...
            img, distance = measurement_cond_fn(x_t=out['sample'],
                                                ...
                                                sample_step = idx,
                                                sigma=sigma,
                                                alphat = torch.tensor(self.alphas[time], device=device, dtype=torch.float32))

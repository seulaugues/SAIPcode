...
from util.img_utils import clip, clear_color, clear, mask_generator, center_crop, normalize_np
...
def main():
    ...
    if measure_config['operator']['name'] == 'inpainting':
        subtask_name = measure_config['mask_opt'].get('mask_type')
        out_path = os.path.join(args.save_dir, 'inpainting', subtask_name)
    else:
    ...
    if measure_config['operator']['name'] == 'super_resolution':
        ...
        if measure_config['operator']['type'] == 'gaussian':
            ...
            kernel1 = torch.Tensor([pdf(-4), pdf(-3), pdf(-2), pdf(-1), pdf(0), pdf(1), pdf(2), pdf(3), pdf(4)]).to(
                device)
            H_funcs = Deblurring2D(kernel1 / kernel1.sum(), kernel2 / kernel2.sum(), model.in_channels,
        ...
        mask_len_range = mask_config.get('mask_len_range', None)
        mask_prob_range = mask_config.get('mask_prob_range', None)
    ...
    for i, ref_img in enumerate(loader):
        ...
        if measure_config['operator']['name'] == 'inpainting':
            ...
            missing_indices = torch.where(mask_flat == 0)[0]
            H_funcs = Inpainting(3, image_size, missing_indices, device)

            # print("[DEBUG] mask is None?", mask is None)

            measurement_cond_fn = partial(cond_method.conditioning, mask=mask)
            sample_fn = partial(sample_fn, measurement_cond_fn=measurement_cond_fn)

            y_n = H_funcs.H(ref_img) + noiser.sigma * torch.randn_like(H_funcs.H(ref_img))

            # print(f"[Inpainting] y_n shape: {y_n.shape}, numel: {y_n.numel()}")

        else:
            # === 其他任务 (super-res, deblur, etc.) ===
            y_x = H_funcs.H(ref_img)
            y_n = y_x + noiser.sigma * torch.randn_like(y_x)
            sample_fn = sample_fn  # 不变

        # === DMPS采样 ===
        DMPS_start_time = time.time()
        x_start = torch.randn(ref_img.shape, device=device).requires_grad_()
        sample = sample_fn(x_start=x_start, measurement=y_n, H_funcs=H_funcs, noise_std=noiser.sigma, record=True,
                           save_root=out_path)
        ...
        ref_lpips = (ref_img * 2 - 1).clamp(-1, 1)
        sample_lpips = (sample * 2 - 1).clamp(-1, 1)
        lpips_score = loss_fn_alex(ref_lpips, sample_lpips).item()
        ...
        if measure_config['operator']['name'] == 'color':
            ...
            mask_expand = mask.repeat(1, 3, 1, 1)

            noise = torch.randn_like(ref_img) * noiser.sigma
            noisy_img = ref_img + noise
            y_n = noisy_img * mask_expand + noise * (1 - mask_expand)
            img_np = y_n[0].permute(1, 2, 0).detach().cpu().numpy()
            img_np = (img_np - img_np.min()) / (img_np.max() - img_np.min() + 1e-10)
            plt.imsave(os.path.join(out_path, 'input', fname), img_np)
    ...
    np.savetxt(os.path.join(out_path, 'lpips_results.csv'), np.array(lpips_results))

    print('Total # images:{}, total  running Time: {}'.format(Num_count, end_time - start_time))

...
class GaussianDiffusion:
    ...
    def p_sample_loop(self,
        ...
        # scale of the pseudo variance
        scale_pseudo_noise = torch.linspace(1, 1, steps=self.num_timesteps).to(device)
        ...
        for idx in pbar:
            ...
            # scale_pseudo_noise[time] = 1.8*(0.2/1.8)**(idx/1000)
            ...
            img = measurement_cond_fn(x_t=out['sample'],
                                      ...
                                      alpha_bar_prev = self.alphas_cumprod_next[time],
                                      alpha_bar = self.alphas_cumprod[time],
                                      pseudonoise_scale = scale_pseudo_noise[time],
                                      noise_prior_score = out['noise_prior_score'],
                                      sample_step = idx,
    ...
    def p_mean_variance(self, model, x, t):
        ...
        if model_output.shape[1] == 2 * x.shape[1]:
            model_output, model_var_values = torch.split(model_output, x.shape[1], dim=1)  # model输出的是noise和方差
        else:
        ...
        noise_prior_score = model_output
        ...
        return {'mean': model_mean,
                ...
                'noise_prior_score': noise_prior_score}
...
class DDPM(SpacedDiffusion):
    def p_sample(self, model, x, t):
        ...
        return {'sample': sample, 'pred_xstart': out['pred_xstart'], 'noise_prior_score': out['noise_prior_score']}
